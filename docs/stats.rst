...
output arrays.
For instance, ``ts.diversity()`` will return a single number (the mean sequence
diversity of all samples), while::

    ts.diversity(samples=[ts.samples()], windows=[0.0, ts.sequence_length])

will return the same number in a ``1x1`` numpy array.
